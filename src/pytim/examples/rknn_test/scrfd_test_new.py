...
def save_detect_faces(detect_faces, ori_file_name, save_file_name):
    src_img = cv2.imread(ori_file_name)
    ...
    cv2.imwrite(save_file_name, src_img)


if __name__ == "__main__":
    test_files = ["scrfd_test.jpg", ]
# convert rknn to timvx engine
    ...
    input_data = cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB)
    input_data = input_data.reshape((1, 480, 320, 3))
    input_data = input_data.transpose((2,1,3,0)) # convert nhwc to whcn
    ...
    output_data_3 = ((output_data_3 - 1.0) * 0.017149630934000015).transpose((2,1,0))
    output_data_4 = ((output_data_4 - 0.0) * 0.03239903599023819).transpose((2,1,0))
    output_data_5 = ((output_data_5 - 0.0) * 0.021522123366594315).transpose((2,1,0))
    output_data_6 = ((output_data_6 - 117.0) * 0.02149081602692604).transpose((2,1,0))
    output_data_7 = ((output_data_7 - 118.0) * 0.03708931431174278).transpose((2,1,0))
    output_data_8 = ((output_data_8 - 118.0) * 0.025925135239958763).transpose((2,1,0))
    ...
    # post process
    rknn_detect_faces = decode(ratio, pad, src_img, src_img, outputs, 0.4, 0.45)
    print("detect {} faces".format(len(rknn_detect_faces)))
    save_detect_faces(rknn_detect_faces, "./examples/rknn_test/scrfd_test.jpg", "./detect_result.jpg")